...
void main() {
  ...
  testWidgets('shows generic invalid message when referral apply fails', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(1170, 2532);
    tester.view.devicePixelRatio = 3;
    ...
    final provider = _FakeReferralRewardsProvider(
      ...
      applyError:
          'PostgrestException(message: duplicate key violates constraint)',
    );
    ...
    expect(provider.appliedReferralCodes, ['BAD-CODE']);
    expect(find.text('Code not valid'), findsOneWidget);
    expect(find.textContaining('PostgrestException'), findsNothing);
    expect(find.textContaining('duplicate key'), findsNothing);
  });
}
