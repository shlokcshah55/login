...
SYSTEM_COLLECTION_NAMES = {'Been To', 'Shared Finds'}
...
class CollectionGenerator:
    ...
    @staticmethod
    def _extract_collection_ids(parsed: object) -> List[str]:
        """
        Accepts either:
          - JSON array of strings: ["date_night", ...]
          - JSON array of objects: [{"id":"date_night", ...}, ...]
          - JSON object with `collections`: {"collections":[{"id":"date_night", ...}, ...], ...}
        Returns a flat list of collection id strings (may include unknown IDs; caller filters).
        """
        ids: List[str] = []
    ...
    async def call_grok_for_location(
        ...
        async with semaphore:
            ...
            try:
                ...
                logger.warning(
                    f"Failed to parse Grok JSON for location {location.get('location_id')}: {e}"
                )
                return []

            ids = self._extract_collection_ids(parsed)
            # Filter to known collection IDs only
            return [cid for cid in ids if cid in COLLECTION_METADATA]
    ...
    def get_saved_locations_not_in_collections(self, collection_ids: List[str]) -> List[Dict]:
        """
        Returns location rows for every location the user has saved that is NOT
        already present in any of the given collection_ids.
        """
        response = self.supabase.rpc('get_saved_locations_not_in_collections', {
            'p_user_id': self.user_id,
    ...
    async def run_auto_update(self) -> Dict:
        """
        Auto-update pipeline: finds every saved location that isn't already in
        one of the user's AI-generated collections, runs them through Grok, and
        adds them to the appropriate collections (creating new ones if needed).
        """
        # Resolve which collections already exist for the known AI labels.
        all_existing = self.get_existing_collections()
        ai_labels = set(COLLECTION_METADATA.values())
        existing_ai: Dict[str, str] = {
            name: cid for name, cid in all_existing.items()
            if name in ai_labels and name not in SYSTEM_COLLECTION_NAMES
        }
        ai_collection_ids = list(existing_ai.values())

        # Find saved locations not yet in any AI collection.
        locations = self.get_saved_locations_not_in_collections(ai_collection_ids)
        logger.info(f"Auto-update: {len(locations)} uncategorised saved location(s) for user {self.user_id}")

        if not locations:
            return {'success': True, 'collections_created': 0, 'collections': []}
        ...
        updated = self.add_to_existing_collections(aggregated, existing_ai)

        self.mark_generated_collections()

        return {
            'success': True,
            'collections_created': len(updated),
            'collections': updated,
        }
