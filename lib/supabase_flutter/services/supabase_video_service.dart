...
class SupabaseVideoService {
  ...
  Future<VideoModel?> submitVideo(
    ...
    try {
      ...
      final videoData = {
        ...
        SupabaseConstants.columnPostedAt: (postedAt ?? DateTime.now()).toIso8601String(),
      };
      
      if (extractedLocationId != null) {
        videoData[SupabaseConstants.columnExtractedLocationId] = extractedLocationId;
      }
  ...
  Future<List<VideoModel>> getUserVideos() async {
    try {
      ...
      return (response as List).map((data) => VideoModel.fromJson(data)).toList();
    } catch (e) {
      if (kDebugMode) {
  ...
  Future<List<VideoModel>> getVideosForLocation(int locationId) async {
    try {
      final response = await _client
          ...
          .order(SupabaseConstants.columnPostedAt, ascending: false);
    ...
    try {
      ...
      await _client
          ...
          .upsert({
            ...
            SupabaseConstants.columnSourceVideoId: videoId,
