...
class HomeController {
  ...
  Future<void> fetchAndPlotRecommendedPins() async {
    ...
    LatLng? currentLocation = deviceLocationProvider.currentPosition ?? await deviceLocationProvider.getCurrentLocation();
  ...
  Widget buildGridItem(BuildContext context, LocationModel location) {
    ...
    return GestureDetector(
      onLongPress: () async {
        ...
        if (isFlipped.value && currentPosition != null && location.position != null) {
      ...
      child: ValueListenableBuilder<bool>(
        ...
        builder: (context, flipped, child) {
          return AnimatedSwitcher(
            ...
            transitionBuilder: (Widget child, Animation<double> animation) {
              ...
              return AnimatedBuilder(
                ...
                builder: (context, child) {
                  ...
                  final rotationValue = isFront ? rotate.value : (pi - rotate.value); // Adjust rotation for back

                  // Prevent interaction during flip
                  return IgnorePointer(
                     ignoring: animation.status == AnimationStatus.forward || animation.status == AnimationStatus.reverse,
                     child: Transform(
            ...
            // Use location.id for a stable key if possible
            child: InkWell(
              key: ValueKey<bool>(flipped), // Key is important for AnimatedSwitcher
              onTap: () async {
              ...
              child: flipped ? _buildBackSide(context, location) : _buildFrontSide(location), // Pass context if needed
            ),
          );
        },
      ),
...
void _adjustCameraToFit(LatLng currentLocation, LatLng targetLocation) {
  mapStateProvider.focusOnBounds(currentLocation, targetLocation); // Use provider method
}
...
void _drawRoute(LatLng start, LatLng end) async {
  ...
  final polyline = Polyline(
    polylineId: const PolylineId('dotted_route'), // Use a constant or unique ID
