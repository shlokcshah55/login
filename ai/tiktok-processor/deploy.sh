...
gcloud run deploy $SERVICE_NAME \
  ...
  --concurrency 10 \
  --max-instances 10 \
  --set-env-vars "PLAYWRIGHT_BROWSERS_PATH=/ms-playwright,PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1,DISPLAY=:99,DBUS_SESSION_BUS_ADDRESS=/dev/null,XAI_API_KEY=${XAI_API_KEY},GOOGLE_PLACES_API_KEY=${GOOGLE_PLACES_API_KEY},SUPABASE_URL=${SUPABASE_URL},SUPABASE_SERVICE_KEY=${SUPABASE_SERVICE_KEY},APPIFY_KEY=${APPIFY_KEY},SEND_PUSH_NOTIF_SECRET=${SEND_PUSH_NOTIF_SECRET}"
