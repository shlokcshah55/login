-- ============================================================================
-- FIX: get_locations_with_pillars KNN ordering + slim projection
-- ============================================================================
-- The v6 RPC is functionally correct but too slow on cold reads:
--   1. It selected `l.*` in both branches, widening every row unnecessarily
--   2. It ordered by `ST_Distance(...)`, which forces Postgres to compute
--      exact distance for every row in the radius and then sort them
--   3. Under Supabase's statement timeout, that shape intermittently fails
--      for dense 15 km searches before eventually succeeding on warm pages
--
-- Keep the same return shape and two-pass inventory policy, but switch both
-- branches to a slim projection and KNN ordering (`<->`) so the GIST indexes
-- on `location_popularity_app.geog` and `locations.geog` can drive the scan.
-- ============================================================================
