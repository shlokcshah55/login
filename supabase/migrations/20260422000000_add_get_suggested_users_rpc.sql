...
BEGIN
  ...
  scored AS (
    SELECT
      ...
      CASE
        ...
        ELSE (
          -- Centered cosine similarity over real[] vectors, normalised to [0, 1].
          -- Centering removes the baseline offset (everyone starts near 50)
          -- so the score reflects genuine taste overlap, not vector magnitude.
          SELECT
            CASE
  ...
  SELECT
    ...
    s.username,
    s.vibe_tag_affinity,
    s.dietary_requirement_tag_affinity,
    s.generated_collections,
